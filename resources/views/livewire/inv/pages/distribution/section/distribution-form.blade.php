...
state(['item_name', 'contact_location', 'assign_contact', 'condition', 'status', 'note', 'quantity']);
...
$assign = function (LivewireAlert $alert) {
    ...
    if ($this->quantity <= $item->stock) {
        ...
        $alert->title('Quantity not available!')->position('top-end')->error()->toast()->timer(3000)->show();
    }
};
