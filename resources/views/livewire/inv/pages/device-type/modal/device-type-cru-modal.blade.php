...
<div>
    <form
        ...
        x-init="init()"
        @device-type-data.window="handleDeviceTypeData($event.detail)"
    ...
    >
        ...
        <x-bale.modal-action>
            <x-bale.button type="submit" label="{{ $edit_mode ? 'update' : 'store' }}" @click="useSpinner()" class="ml-3" />
            <x-bale.secondary-button label="Cancel" type="button" wire:click="$dispatch('closeBaleModal', { id: 'deviceTypeModal' }); $wire.resetVal()" />
        </x-bale.modal-action>
    </form>
</div>
