...
state(["availableDeviceTypes" => fn () => DeviceType::get()]);
...
<div>
    ...
    <x-bale.page-container>
        ...
        <form
            wire:submit="{{ $edit_mode ? " update" : "store" }}"
            class="mt-8"
            x-data="{
                itemType: $wire.entangle('item_type'),
                itemDeviceType: $wire.entangle('item_device_type'),
            }"
        >
            {{-- item name --}}
            <div class="mb-4">
                <x-bale.input type="text" class="sm:w-1/4" label="Item Name" wire:model="item_name" autocomplete="off" name="item_name" autofocus />
                    ...
                    <x-bale.select-dropdown label="select type">
                        ...
                        @foreach ($availableCategories as $category)
                            <label
                                ...
                                wire:key="{{ $category["name"] }}"
                                @click="title='{{ $category["name"] }}'"
                            >
            ...
            {{-- brand --}}
            <div class="mb-4">
                <x-bale.input type="text" label="Brand" wire:model="item_brand" autocomplete="off" name="item_brand" />
                <x-input-error for="item_brand" />
            </div>

            {{-- specification --}}
            <div class="mb-4">
                <label for="specification" class="block w-full mb-2 text-sm font-medium dark:text-white">Specification (Optionasl)</label>
                <textarea
                    id="specification"
                ...
                ></textarea>
                <p class="mt-1 text-xs text-gray-500 dark:text-neutral-500" id="specification">Insert specification</p>

                <x-input-error for="specification" />
            </div>

            <div class="mb-4" x-show="itemType == 'software'" wire:transition>
                <x-bale.input type="text" label="License purpose" wire:model="item_license_purpose" autocomplete="off" name="item_license_purpose" />
                <x-input-error for="item_license_purpose" />
            </div>

            <div class="sm:w-1/4" x-show="itemType == 'hardware'" wire:transition>
                <x-bale.select-dropdown label="Select Device">
                    <x-slot name="defaultValue">
                        <span x-text="itemDeviceType == null ? 'Open this select menu' : itemDeviceType"></span>
                    </x-slot>
                    @foreach ($availableDeviceTypes as $device_type)
                        <label
                            for="{{ $device_type["device_name"] }}"
                            class="flex w-full p-3 text-sm transition duration-200 ease-out bg-white hover:bg-gray-200 hover:rounded-lg dark:bg-neutral-900 hover:dark:border-neutral-700 dark:text-neutral-400"
                            wire:key="{{ $device_type["device_name"] }}"
                            @click="title='{{ $device_type["device_name"] }}'"
                        >
                            <span class="text-sm text-gray-500 dark:text-neutral-400">{{ $device_type["device_name"] }}</span>
                            <input
                                type="radio"
                                name="item_device_type"
                                wire:model="item_device_type"
                                value="{{ $device_type["device_name"] }}"
                                class="shrink-0 ms-auto mt-0.5 border-gray-200 rounded-full text-blue-600 focus:ring-blue-500 disabled:opacity-50 disabled:pointer-events-none dark:bg-neutral-800 dark:border-neutral-700 dark:checked:bg-blue-500 dark:checked:border-blue-500 dark:focus:ring-offset-gray-800"
            ...
            <x-bale.modal-action>
                <x-bale.button label="{{ $edit_mode ? 'Update' : 'Store' }}" type="submit" wire:dirty.attr="disabled" wire:target="store, update" />
            </x-bale.modal-action>
        </form>
    </x-bale.page-container>
</div>
